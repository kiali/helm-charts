...
{{/*
Determine if using a built in or autogenerated certificate.
*/}}
{{- define "kiali-server.identity.provider" }}
  {{- if .Capabilities.APIVersions.Has "route.openshift.io/v1" }}
    openshift/v1
  {{- else if .Values.identity.cert_manager.enabled }}
    cert_manager
  {{- else }}
  ""
  {{- end }}
...
{{- else }}
  {{- if (include "kiali-server.identity.provider" .) }}
    {{- "/kiali-cert/tls.crt" }}
  {{- else }}
    {{- "" }}
  {{- end }}
